...
export interface SortType {
  /**
   * The column name.
   * This is the same as `header.prop` from `TableColumnInfo` interface.
   */
  prop: string;
  /** The sort order. */
  order: 'asc' | 'desc';
 ...
 * @deprecated
 *
 * This type is deprecated. Consider using `TableColumn` instead.
 *
...
/**
 * The data structure of a grouped column.
 */
export type TableGroup<T = any> = {
  type: 'group';
  title: string;
  /** An array of nested header children, which can be yet another group column. */
  headerChildren: TableColumn<T>[];
};

/**
 * The data structure of a column, which will be used for the `headers`
 * prop of the `DatatableWrapper` props, as well as the `TableHeader` props.
 */
export type TableColumnInfo<T = any> = {
  type?: 'column';
  /**
   * The prop name for the header. This prop should exist in the table body's data
   * so that the column can render a non-empty cell. Moreover, each header should
...
/**
 * The type for the table column, which can be either a group or an actual column.
 */
export type TableColumn<T = any> = TableGroup<T> | TableColumnInfo<T>;
